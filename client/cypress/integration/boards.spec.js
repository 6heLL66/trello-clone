...
import { methods } from '../../src/constants/values'
import { boardCreatedAlert, boardDeletedAlert } from '../../src/constants/alerts'
...
describe("Boards Tests", () => {
  it('Login and Visit Boards Page', () => {
    ...
    cy.visit(loginPage)
    cy.window().its('store')
      .invoke('dispatch', login('test_user', '123456'))
    ...
    cy.get('.board-button:first-child')
      .click()

    cy.get('.board-create-form').should('exist')

    cy.get('.board-create-form > div > svg')
      .click()
    ...
    cy.get('#board_name')
      .type('test_board')
      .should('have.value', 'test_board')

    cy.get('.board-create-form > div > button')
      .click()
    cy.wait('@createBoard')

    cy.contains('test_board').should('exist')

    cy.get('.board-button:last-child > div > svg')
      .click()
    cy.wait('@deleteBoard')
